...
%    zz =  NonCrossable_PF(X(1, i), X(3, i),p(n_states * 2+ n_controls+1), x_y_1, X(2, i),X(4, i), V_1, 2, 1);
    obstacle_position = p(n_states * 2+ n_controls+1) + N*V_1;
   %continously update 
    if i == 1
        st = X(:, i);
        con = U(:, i); 
        zz =  500 * NonCrossable_PF(X(1, i), X(3, i),p(n_states * 2+ n_controls+1), x_y_1, X(2, i),X(4, i), V_1, 2, 1);
        ...
        st = X(:, i);
        con = U(:, i);  
        zz =  500 * NonCrossable_PF(X(1, i), X(3, i),obstacle_position, x_y_1, X(2, i),X(4, i), V_1, 2, 1);
        obj = obj + zz + (st - p((n_states + 1):(n_states * 2)))' * Q *...
            (st - p((n_states + 1):(n_states * 2))) + con' * R * con +...
